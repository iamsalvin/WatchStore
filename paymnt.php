...
if (isset($_POST['submit']))
{
   ...
   $c=$_POST['cno'];
   $sdate=date('Y-m-d');
   $sno=mysqli_query($con,"select max(sno)+1 from invoice");
   $nextno=mysqli_fetch_array($sno);
   if(empty($nextno[0]))
   {
    $no=1;
   }
   ...
   $n=mysqli_query($con,"insert into invoice(sno,sdate,custmid,name,shipadd,total,scharge,gtotal,cardno,status) values('$no','$sdate','$un','$nm','$s','$tot',40,'$g','$c','new order')");
   ...
   while($res=mysqli_fetch_array($data))
   {
     ...
     $to=$res['tot'];
     $ph=$res['photo'];
     $n=mysqli_query($con,"insert into invoiceitem(sno,slno,pname,quantity,price,total,photo) values ('$no','$slno','$pn','$qty','$pr','$to','$ph')")  ;
     $n=mysqli_query($con,"update addprod set stock= stock -'$qty' where pn='$pn'");

     $slno=$slno+1;

   }
   $n=mysqli_query($con,"delete from cart where uname='$un'");
...
<div>
  <form  class="form" method="POST">
    <div>
      ...
      <div class="card">
        <address>
         <textarea rows="6" cols="35" name="s1">

         </textarea>

        </address>
      </div>
      card no <input type="text" name="cno" maxlength="12" required >
    </div>
    ...
    <div>
      <button class="button button--full" type="submit" name="submit"><svg class="icon">
